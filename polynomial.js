...
const a = tf.variable(tf.scalar(Math.random()/100));
const b = tf.variable(tf.scalar(Math.random()/100));
const c = tf.variable(tf.scalar(Math.random()/100));
const d = tf.variable(tf.scalar(Math.random()/100));
const e = tf.variable(tf.scalar(Math.random()/100));

// Step 2. Create an optimizer, we will use this later. You can play
// with some of these values to see how the model performs.
const numIterations = 1000;
const learningRate = 0.07427512501934785;
...
async function learnCoefficients() {
  const trueCoefficients = {a: -.8, b: -.2, c: .9, d: .5, e: .9};
  ...
  const predictionsBefore = predict(trainingData.ys);
  await plotDataAndPredictions(
      '#random .plot', trainingData.ys, trainingData.xs, predictionsBefore);

  // Train the model!
  await train(trainingData.ys, trainingData.xs, numIterations);
  ...
  const predictionsAfter = predict(trainingData.ys);
  await plotDataAndPredictions(
